...
use Usjnet\Sso\Support\AuthenticatesLocalGuardRequest;
use Usjnet\Sso\Support\AuthenticatesSsoRequest;
use Usjnet\Sso\Support\HandlesSsoLogout;

/**
 * Allow either a local session guard (e.g. admin/web) or a valid SSO access cookie.
 */
class EnsureHybridWebAuthenticated
{
    use AuthenticatesLocalGuardRequest;
    ...
    {
        if ($this->authenticateLocalGuardRequest($request)) {
            return $next($request);
        }
